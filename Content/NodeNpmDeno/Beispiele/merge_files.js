class FileMerger{
    ...
    constructor() {
        ...
        var merged_lines= [];
        console.time("test");
        fs.readFile('big_file1.txt','utf8', callback(lines1)); //Both files have 1 Million rows
        fs.readFile('big_file2.txt','utf8', callback(lines2));
        
        //alternative zu callback -> promisies require('fs').promises;
        function callback(lines) {  
            return function(err, data) {
                lines.push(data.split("\n"));   //fill array with lines
                if( lines1.length && lines2.length ){
                    lines1[0].forEach((line,i) => merged_lines[i]= line +" "+lines2[0][i] ); //lines and lines_1, lines_2 are 2 dim array
